function [f_inf] = QCM(rho0, Jz0, kappa0, R, Rmat, Zmat)

    dr = Rmat(2, 1) - Rmat(1, 1);
    dz = Zmat(1, 2) - Zmat(1, 1);
    ...
    % disp('Mk');
    % disp([n_k; u_para_k; T_k]);
    % disp('f_inf moments');
    % 
    f_inf = (n_k/((2*pi*R*T_k)^(3/2)))*exp(-((Zmat-u_para_k).^2 + Rmat.^2)/(2*R*T_k));
    % rho = sum(sum(f_inf.*Rmat))*2*pi*dr*dz;
    % Jz  = sum(sum(f_inf.*Rmat.*Zmat))*2*pi*dr*dz;
    % kappa   = sum(sum(f_inf.*Rmat.*((Rmat.^2 + Zmat.^2)/2)))*2*pi*dr*dz;
    % 
    % disp([rho; Jz; kappa]);
end