...
DTvals = [0.4];
Nx =  80;
ERRORS = zeros(4 + Nx, numel(DTvals));
...
for t_index = 2:Nt
    ...
    [n_vals, nu, nU, Te_vals, u_hat, nu_hat, S_hat, Q_hat, nTe_hat, kappaTx] = fluid_solver(f_vals_low_rank, n_vals, u_para, U, Te_vals, dt, dx, dr, dz, Rmat, Zmat, qa, qe, ma, me, R_const, x_min, x_max);
...
for j = 1:Nx
    ERRORS(4 + j, dtIndex) = 2*pi*dr*dz*sum(sum(abs(f_vals_low_rank{j, 1}*f_vals_low_rank{j, 2}*f_vals_low_rank{j, 3}' - soln{j, 1}*soln{j, 2}*soln{j, 3}')));
end
end
%%
...
for j = 1:Nx
    ...
    disp(ERRORS(4 + j, :)');
    disp(log2(ERRORS(4 + j, 1:end-1)./ERRORS(4 + j, 2:end))');
end
...
% ------- FLUID SOLVER STUFF -------
function [n, nu, nU, T_e, u_para0_half_nodes, nu_hat, S_hat, Q_hat, nTe_hat, kappaTx] = fluid_solver(f_vals_low_rank, n0, u_para0, U0, T_e0, dt, dx, dv_perp, dv_para, V_perp, V_para, qa, qe, ma, me, R_const, x_min, x_max)
    ...
    [nu_hat, S_hat, Q_hat] = get_fluxes(f_vals_low_rank, V_perp, V_para, R_const, x_min, x_max, dx, dv_perp, dv_para);
    ...
    u_para0_half_nodes = u_para0_half_nodes;
    Te_all = [T_ae_min; T_e; T_ae_max];
    % kappaTx = (3.2/(2*sqrt(2*me)))*((Te_pos.^(5/2) + Te_neg.^(5/2))) .* ((1/dx)*(Te_pos - Te_neg));
