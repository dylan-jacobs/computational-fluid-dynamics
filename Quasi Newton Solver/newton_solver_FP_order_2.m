...
function [n2, u_para2, T_a2, T_e2] = newton_solver_FP_order_2(f, n0, u_para0, T_a0, T_e0, dt, dx, dv_para, dv_perp, v_para, v_perp, qa, qe, ma, me, R_const, x_min, x_max)
    ...
    [nu_hat2, S_hat2, Q_hat2] = get_fluxes(f1, v_perp, v_para, R_const, x_min, x_max, dx, dv_perp, dv_para);
...
function [nu_hat, S_hat, Q_hat] = get_fluxes(f, V_perp, V_para, R_const, x_min, x_max, dx, dv_perp, dv_para)
    Nx = size(f, 3);
    [n0, u_para0, T0] = get_boundaries();

    leftBC = maxwellian(n0(x_min - dx/2), V_para, V_perp, u_para0(x_min - dx/2), T0(x_min - dx/2), R_const);
    rightBC = maxwellian(n0(x_max + dx/2), V_para, V_perp, u_para0(x_max + dx/2), T0(x_max + dx/2), R_const);
