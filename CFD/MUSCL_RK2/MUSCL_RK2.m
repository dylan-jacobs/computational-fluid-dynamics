...
tf = 1; % final time --> choose BEFORE breaking time
f = @(u) (u.^2)./2;
%f = @(u) u;
...
title('MUSCL + RK2 Temporal Convergence at t=0.5'); xlabel('\lambda'); ylabel('L1 Error');
...
figure(2); clf;
xmid = -2*pi:dx:pi/2; xmid = xmid(1:end-1) + dx/2;
plot(xmid+(tf*u_exact_eqn(xmid)), u_exact_eqn(xmid) , 'black--'); hold on;
